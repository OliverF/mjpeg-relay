    ...
    def handleRequest(self, clientsock):
        ...
        if (buff != ""):
            ...
            if ("/status" in requestPath):
                ...
                clientsock.sendall(
                    self.statusHTML.format(clientcount=self.broadcast.getClientCount(),
                                           bwin=float(self.status.bandwidthIn * 8) / 1000000,
                                           bwout=float(self.status.bandwidthOut * 8) / 1000000))
                clientsock.close()
                ...
                if (self.broadcast.broadcasting):
                    clientsock.sendall(
                        self.dummyHeader.format(boundaryKey=self.broadcast.boundarySeparator))
